...
class App(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        if True:
            ...
            self.setGeometry(
                QStyle.alignedRect(
                    Qt.LeftToRight,
                    Qt.AlignHCenter,
                    QSize(self.windowWidthOriginal, self.windowHeightOriginal),
                    screen,
                )
            )
        ...
        if True:
            try:
                ...
                _logger.critical(
                    "Cannot load driver definitions: " + traceback.format_exc()
                )
        ...
        if True:
            try:
                ...
                try:
                    ...
                    try:
                        _logger.info(
                            "  .. moving settings file to /config/settings.json.. "
                        )
                        ...
                        _logger.warning(
                            "Cannot rename old settings.json, leaving in place and using new file."
                        )
                except FileNotFoundError:
                    # create config folder if it doesn't exist
                    os.makedirs("./config", exist_ok=True)
                    ...
                    try:
                        ...
                        self._cameraWidth = int(
                            self.__userSettings["camera"][0]["display_width"]
                        )
                        self._cameraHeight = int(
                            self.__userSettings["camera"][0]["display_height"]
                        )
                        ...
                        errorMsg = (
                            "Error reading user settings file." + traceback.format_exc()
                        )
        ...
        if True:
            ...
            if self.__activePrinter["controller"] == "RRF":
                (_errCode, _errMsg, self.printerURL) = self.sanitizeURL(
                    self.__activePrinter["address"]
                )
    ...
    def setupMainWindow(self):
        ...
        if True:
            ...
            if True:
                ...
                self.connectButton.setMinimumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.connectButton.setMaximumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.connectButton.setToolTip("Connect..")
                self.connectButton.setDisabled(True)
                self.connectButton.clicked.connect(self.connectPrinter)
                self.leftToolbarLayout.addWidget(
                    self.connectButton
                )  # , 1, 0, 1, 1, Qt.AlignLeft|Qt.AlignTop)
                # Disconnect button
                self.disconnectButton = QPushButton("D")
                self.disconnectButton.setStyleSheet(self.styleDisabled)
                self.disconnectButton.setMinimumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.disconnectButton.setMaximumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.crosshairDisplayButton.setMinimumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.crosshairDisplayButton.setMaximumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.cpSetupButton.setMinimumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.cpSetupButton.setMaximumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.cpAutoCaptureButton.setMinimumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                self.cpAutoCaptureButton.setMaximumSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
            ...
            if True:
                ...
                self.mainLayout.addWidget(
                    self.image
                )  # , 1, 1, 1, -1, Qt.AlignLeft|Qt.AlignTop)
            ...
            if True:
                ...
                self.button_x_left.setFixedSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.button_x_right.setFixedSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.button_y_left.setFixedSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.button_y_right.setFixedSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
                ...
                self.button_z_down.setFixedSize(
                    self.pushbuttonSize, self.pushbuttonSize
                )
        ...
        if True:
            ...
            self.footerLayout.addWidget(
                self.instructionsBox, 0, 0, 1, -1, Qt.AlignLeft | Qt.AlignVCenter
            )
            ...
            self.manualCPCaptureButton.setMinimumSize(
                self.pushbuttonSize * 2, self.pushbuttonSize
            )
            self.manualCPCaptureButton.setMaximumSize(
                self.pushbuttonSize * 2, self.pushbuttonSize
            )
            self.manualCPCaptureButton.setToolTip(
                "Capture current machine coordinates as the Control Point."
            )
            ...
            self.footerLayout.addWidget(
                self.manualCPCaptureButton, 0, 1, 1, 1, Qt.AlignRight | Qt.AlignVCenter
            )
            ...
            self.overrideManualOffsetCaptureButton.setMinimumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.overrideManualOffsetCaptureButton.setMaximumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.overrideManualOffsetCaptureButton.setToolTip(
                "Capture current position and calculate tool offset."
            )
            self.overrideManualOffsetCaptureButton.setDisabled(True)
            self.overrideManualOffsetCaptureButton.setVisible(False)
            self.overrideManualOffsetCaptureButton.clicked.connect(
                self.overrideManualToolOffsetCapture
            )
            self.footerLayout.addWidget(
                self.overrideManualOffsetCaptureButton,
                0,
                1,
                1,
                1,
            ...
            self.manualToolOffsetCaptureButton.setMinimumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.manualToolOffsetCaptureButton.setMaximumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.manualToolOffsetCaptureButton.setToolTip(
                "Capture current position and calculate tool offset."
            )
            self.manualToolOffsetCaptureButton.setDisabled(True)
            self.manualToolOffsetCaptureButton.setVisible(False)
            self.manualToolOffsetCaptureButton.clicked.connect(
                self.manualToolOffsetCapture
            )
            self.footerLayout.addWidget(
                self.manualToolOffsetCaptureButton,
                0,
                0,
                1,
                1,
            ...
            self.startAlignToolsButton.setMinimumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.startAlignToolsButton.setMaximumSize(
                self.pushbuttonSize * 3, self.pushbuttonSize
            )
            self.startAlignToolsButton.setToolTip(
                "Start automated tool offset calibration"
            )
            ...
            self.footerLayout.addWidget(
                self.startAlignToolsButton, 0, 1, 1, 1, Qt.AlignRight | Qt.AlignVCenter
            )

            # Resume auto alignment button
            self.resumeAutoToolAlignmentButton = QPushButton("Resume Auto Align")
            self.resumeAutoToolAlignmentButton.setMinimumSize(
                self.pushbuttonSize * 4, self.pushbuttonSize
            )
            self.resumeAutoToolAlignmentButton.setMaximumSize(
                self.pushbuttonSize * 4, self.pushbuttonSize
            )
            self.resumeAutoToolAlignmentButton.setToolTip(
                "Resume automated calibration"
            )
            ...
            self.footerLayout.addWidget(
                self.resumeAutoToolAlignmentButton,
                0,
                0,
                1,
                1,
    ...
    def xleftClicked(self):
        ...
        params = {
            ...
            "position": {"X": str(-1 * incrementDistance)},
        }
    ...
    def xRightClicked(self):
        ...
        params = {
            "moveSpeed": self.__moveSpeed,
            "position": {"X": str(incrementDistance)},
        }
        self.moveRelativeSignal.emit(params)
    ...
    def yleftClicked(self):
        ...
        params = {
            "moveSpeed": self.__moveSpeed,
            "position": {"Y": str(-1 * incrementDistance)},
        }
        self.moveRelativeSignal.emit(params)
    ...
    def yRightClicked(self):
        ...
        params = {
            "moveSpeed": self.__moveSpeed,
            "position": {"Y": str(incrementDistance)},
        }
        self.moveRelativeSignal.emit(params)
    ...
    def zleftClicked(self):
        ...
        params = {
            "moveSpeed": self.__moveSpeed,
            "position": {"Z": str(-1 * incrementDistance)},
        }
        self.moveRelativeSignal.emit(params)
    ...
    def zRightClicked(self):
        ...
        params = {
            "moveSpeed": self.__moveSpeed,
            "position": {"Z": str(incrementDistance)},
        }
        self.moveRelativeSignal.emit(params)
    ...
    def stateConnected(self):
        ...
        numTools = max(self.__activePrinter["tools"], key=lambda x: int(x["number"]))[
            "number"
        ]
        ...
        for tool in range(numTools + 1):
            ...
            if any(d.get("number", -1) == tool for d in self.__activePrinter["tools"]):
                ...
                toolTip += "<pre>"
                toolTip += "X: " + "{:>9.3f}".format(
                    self.__activePrinter["tools"][currentTool]["offsets"][0]
                )
                toolTip += "<br>"
                toolTip += "Y: " + "{:>9.3f}".format(
                    self.__activePrinter["tools"][currentTool]["offsets"][1]
                )
                toolTip += "<br>"
                toolTip += "Z: " + "{:>9.3f}".format(
                    self.__activePrinter["tools"][currentTool]["offsets"][2]
                )
                toolTip += "</pre>"
                currentTool += 1
        ...
        for i, button in enumerate(self.toolButtons):
            ...
            self.jogPanel.addWidget(
                button, (7 + i), 0, Qt.AlignCenter | Qt.AlignHCenter
            )
        ...
        for i, checkbox in enumerate(self.toolCheckboxes):
            ...
            # button.clicked.connect(self.callTool)
            self.jogPanel.addWidget(
                checkbox, (7 + i), 1, Qt.AlignCenter | Qt.AlignHCenter
            )
    ...
    def manualToolOffsetCapture(self):
        ...
        self.manualToolOffsetCaptureButton.setStyleSheet(self.styleDisabled)
        params = {
            "toolIndex": int(self.__activePrinter["currentTool"]),
            "position": self.__currentPosition,
            "cpCoordinates": self.__cpCoordinates,
            "continue": False,
        }
        ...
        if self.uv is not None:
            if self.uv[0] is not None and self.uv[1] is not None:
                ...
                if self.state == 0:
                    ...
                    self.space_coordinates.append(
                        (self.__currentPosition["X"], self.__currentPosition["Y"])
                    )
                    ...
                    if self.state != self.lastState and self.state < len(
                        self.calibrationCoordinates
                    ):
                        _logger.debug(
                            "*** State: "
                        ...
                        if self.olduv is not None:
                            if (
                                self.olduv[0] == self.uv[0]
                                and self.olduv[1] == self.uv[1]
                            ):
                                self.repeatCounter += 1
                        ...
                        if self.state == 1:
                            self.mpp = np.around(
                                0.5
                                / self.getDistance(
                                    self.olduv[0], self.olduv[1], self.uv[0], self.uv[1]
                                ),
                                3,
                            )
                    ...
                    # otherwise, move the carriage from the original position to the next step and increment state
                        ...
                        self.updateStatusbarMessage(
                            "Calibrating camera step " + str(self.state) + ".."
                        )
                        _logger.debug(
                            "Step " + str(self.state) + " detection UV: " + str(self.uv)
                        )
                    ...
                    _logger.info(
                        "Camera calibrated ("
                        + str(cameraCalibrationTime)
                        + "s); aligning.."
                    )

                    # Calculate transformation matrix
                    self.transform_input = [
                        (self.space_coordinates[i], self.normalize_coords(camera))
                        for i, camera in enumerate(self.camera_coordinates)
                    ]
                    (
                        self.transformMatrix,
                        self.transform_residual,
                    ) = self.least_square_mapping(self.transform_input)

                    # define camera center in machine coordinate space
                    self.newCenter = self.transformMatrix.T @ np.array(
                        [0, 0, 0, 0, 0, 1]
                    )
                    self.guessPosition[0] = np.around(self.newCenter[0], 3)
                    self.guessPosition[1] = np.around(self.newCenter[1], 3)
                    _logger.info(
                        "Calibration positional guess: " + str(self.guessPosition)
                    )
                    ...
                    params = {
                        "position": {
                            "X": self.guessPosition[0],
                            "Y": self.guessPosition[1],
                        }
                    }
                    ...
                    if self.__stateEndstopAutoCalibrate:
                        updateMessage = (
                            "Endstop calibration step "
                            + str(self.calibrationMoves)
                            + ".. (MPP="
                            + str(self.mpp)
                            + ")"
                        )
                        ...
                        updateMessage = (
                            "Tool "
                            + str(self.__activePrinter["currentTool"])
                            + " calibration step "
                            + str(self.calibrationMoves)
                            + ".. (MPP="
                            + str(self.mpp)
                    ...
                    _logger.debug(
                        ...
                        + str(self.retries)
                        + " X"
                        + str(self.__currentPosition["X"])
                        + " Y"
                        + str(self.__currentPosition["Y"])
                        + " UV: "
                        + str(self.uv)
                        + " old UV: "
                        + str(self.olduv)
                    ...
                    # Otherwise, check if we're not aligned to the center
                        ...
                        self.olduv = self.uv
                        params = {
                            "position": {"X": self.offsets[0], "Y": self.offsets[1]},
                            "moveSpeed": 1000,
                        }
                        self.moveRelativeSignal.emit(params)
                        _logger.debug(
                    ...
                    # finally, we're aligned to the center, and we should update the tool offsets
                        ...
                        if self.__stateEndstopAutoCalibrate:
                            ...
                            self.__displayCrosshair = False
                            # Set CP location
                            self.__cpCoordinates["X"] = np.around(
                                self.__currentPosition["X"], 2
                            )
                            self.__cpCoordinates["Y"] = np.around(
                                self.__currentPosition["Y"], 2
                            )
                            self.__cpCoordinates["Z"] = np.around(
                                self.__currentPosition["Z"], 2
                            )
                            # Update GUI statusbar with CP coordinates and green status
                            self.cpLabel.setText(
                                "<b>CP:</b> <i>("
                                + str(self.__cpCoordinates["X"])
                                + ", "
                                + str(self.__cpCoordinates["Y"])
                                + ")</i>"
                            )
    ...
    def createDetectionManagerThread(self, announce=True):
        ...
        self.detectionManager = DetectionManager(
            videoSrc=self._videoSrc,
            width=self._cameraWidth,
        ...
        self.setImagePropertiesSignal.connect(
            self.detectionManager.relayImageProperties
        )
        self.resetImageSignal.connect(self.detectionManager.relayResetImage)
        # Endstop alignment signals and slots
        self.toggleEndstopDetectionSignal.connect(
            self.detectionManager.toggleEndstopDetection
        )
        self.toggleEndstopAutoDetectionSignal.connect(
            self.detectionManager.toggleEndstopAutoDetection
        )
        # Nozzle alignment signals and slots
        self.toggleNozzleDetectionSignal.connect(
            self.detectionManager.toggleNozzleDetection
        )
        self.toggleNozzleAutoDetectionSignal.connect(
            self.detectionManager.toggleNozzleAutoDetection
        )
        # UV coordinates update signals and slots
        self.getUVCoordinatesSignal.connect(self.detectionManager.sendUVCoorindates)
        self.detectionManager.detectionManagerUVCoordinatesSignal.connect(
            self.saveUVCoordinates
        )
    ...
    def createPrinterManagerThread(self, announce=True):
        ...
        try:
            ...
            self.printerManager.updateStatusbarSignal.connect(
                self.updateStatusbarMessage
            )
            self.printerManager.activePrinterSignal.connect(self.printerConnected)
            self.printerManager.printerDisconnectedSignal.connect(
                self.printerDisconnected
            )
    ...
    def connectPrinter(self):
        ...
        self.connection_dialog = ConnectionDialog(
            parent=self,
            newPrinter=False,
            settings=self.__userSettings,
            stylesheet=self.globalStylesheet,
        )
        ...
        if connectionDialogReturn == -2:
            ...
            self.__activePrinter = self.__userSettings["printer"][
                connectionDialogReturn
            ]
            try:
    ...
    def haltPrinterOperation(self, **kwargs):
        ...
        # Cancel CP Capture
        if (
            self.__stateAutoCPCapture is True
            or self.__stateEndstopAutoCalibrate is True
            or self.__stateSetupCPCapture is True
        ):
            self.haltCPAutoCapture()
            return
        # Cancel Nozzle Detection
        if (
            self.__stateAutoNozzleAlignment is True
            or self.__stateOverrideManualNozzleAlignment is True
        ):
            self.haltNozzleCapture()
            return
        ...
        # restore position
        if (
            self.__cpCoordinates["X"] is not None
            and self.__cpCoordinates["Y"] is not None
        ):
    ...
    def printerConnected(self, printerJSON):
        ...
        self.connectionStatusLabel.setText(
            self.__activePrinter["nickname"]
            + " ["
            + self.__activePrinter["controller"]
            + " v"
            + self.__activePrinter["version"]
            + "]"
        )
    ...
    def printerError(self, message):
        ...
        try:
            ...
            _logger.warning("Printer thread not created yet.")
    ...
    def toolLoaded(self):
        ...
        if (
            ...
            params = {
                ...
                "moveSpeed": 5000,
                "position": {
                    "X": self.__cpCoordinates["X"],
                    "Y": self.__cpCoordinates["Y"],
                    "Z": self.__cpCoordinates["Z"],
                },
            }
        else:
            params = {
                ...
                "moveSpeed": 5000,
                "position": {
                    "X": self.__currentPosition["X"],
                    "Y": self.__currentPosition["Y"],
                    "Z": self.__currentPosition["Z"],
                },
            }
        self.moveAbsoluteSignal.emit(params)
    ...
    def registerActiveTool(self, toolIndex):
        ...
        self.__mutex.unlock()
        if (
            self.__stateAutoNozzleAlignment is False
            and self.__stateOverrideManualNozzleAlignment is False
            and int(toolIndex) != -1
        ):
            self.startAlignToolsButton.setVisible(False)
            if (
                self.__cpCoordinates is not None
                and self.__cpCoordinates["X"] is not None
            ):
    ...
    def saveUVCoordinates(self, uvCoordinates):
        ...
        if (
            self.__stateEndstopAutoCalibrate is True
            or self.__stateAutoNozzleAlignment is True
        ):
            if uvCoordinates is None:
                # failed to detect, poll coordinates again
                self.retries += 1
                self.startAlignToolsButton.setText(
                    "Retrying (" + str(self.retries) + ").."
                )
        ...
        if self.__stateManualCPCapture is True:
            ...
            if self.state != 100:
                if int(self.__activePrinter["currentTool"]) > -1:
                    _logger.debug(
                        "saveCurrentPosition: autoCalibrate nozzle for T"
                        + str(int(self.__activePrinter["currentTool"]))
                    )
                else:
                    _logger.debug("saveCurrentPosition: autoCalibrate endstop.")
            elif self.state == 100:
                _logger.debug(
                    "saveCurrentPosition: autoCalibrate nozzle set offsets for T"
                    + str(int(self.__activePrinter["currentTool"]))
                )
            ...
            _logger.debug(
                "saveCurrentPosition: manual nozzle set offsets for T"
                + str(int(self.__activePrinter["currentTool"]))
            )
            # set state to detect next tool
            self.state = 200
    ...
    def displayPreferences(self, event=None, newPrinterFlag=False):
        ...
        try:
            self.settingsDialog = SettingsDialog(
                parent=self,
                newPrinter=newPrinterFlag,
                geometry=self.__settingsGeometry,
                settings=self.__userSettings,
                firmwareList=self.__firmwareList,
                cameraProperties=self.__activeCamera,
            )
            # # Signals
            ...
            self.settingsDialog.settingsResetImageSignal.connect(
                self.relayResetCameraDefaults
            )
            self.settingsDialog.settingsSetImagePropertiesSignal.connect(
                self.relayImageParameters
            )
            self.settingsDialog.settingsUpdateSignal.connect(self.updateSettings)
            self.settingsDialog.settingsStatusbarSignal.connect(
                self.updateStatusbarMessage
            )
            self.settingsDialog.settingsGeometrySignal.connect(
                self.saveSettingsGeometry
            )
    ...
    def toggleCrosshair(self):
        if (
            self.__stateAutoCPCapture is False
            and self.__stateAutoNozzleAlignment is False
            and self.__stateEndstopAutoCalibrate is False
        ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Program to allign multiple tools on Duet/klipper based printers, using machine vision.",
        allow_abbrev=False,
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Enable debug output to terminal"
    )
    ...
    fileFormatter = logging.Formatter(
        "%(asctime)s --- %(levelname)s --- M:%(name)s --- T:%(threadName)s --- F:%(funcName)s --- L:%(lineno)d --- %(message)s"
    )
    ...
    print()
    _logger.warning(
        "This is an alpha release. Always use only when standing next to your machine and ready to hit EMERGENCY STOP."
    )
    print()
